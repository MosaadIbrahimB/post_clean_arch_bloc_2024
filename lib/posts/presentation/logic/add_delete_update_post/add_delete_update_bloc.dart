...
class AddDeleteUpdatePost
  ...
  AddDeleteUpdatePost({required this.addPost, required this.deletePost, required this.updatePost})
    ...
    on(
      (event, emit) async {
        if (event is AddEvent) {
          ...
          emit(_getState(response, "Success Add post"));
        } else if (event is UpdateEvent) {
          emit(LoadingAddUpdateDeleteState());
          Either<Failure, Unit> response = await updatePost(event.post);
          emit(_getState(response, "Success Update post"));
        } else if (event is DeleteEvent) {
          emit(LoadingAddUpdateDeleteState());
          Either<Failure, Unit> response = await deletePost(event.postId);
          emit(_getState(response, "Success Delete post"));
        }
      },
    );
  }
